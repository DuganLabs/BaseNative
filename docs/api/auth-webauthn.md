...
```bash
npm install @basenative/auth-webauthn @simplewebauthn/server
# Browser-side only:
...
- `opts.userVerification` — `'preferred'` (default) | `'required'` | `'discouraged'`. Drives both option generation (`authenticatorSelection.userVerification` on registration, `userVerification` on authentication) and the verification step (`requireUserVerification: userVerification === 'required'`), so the two can never disagree. See the security note under `verifyRegistration`/`verifyAuthentication` below.
...
Throws synchronously if `opts.rp.rpID`/`opts.rp.origin` or `opts.stores` are missing, if `opts.stores` is missing any required method, or if `opts.userVerification` is set to anything other than `'preferred'`, `'required'`, or `'discouraged'`.
...
- **`getRegistrationOptions(rawHandle)`** — normalizes and validates the handle (`{ error: 'bad-handle', status: 400 }` if invalid); looks up or creates the user; excludes the user's existing credentials via `excludeCredentials`; calls `generateRegistrationOptions` with `attestationType: 'none'` and `authenticatorSelection: { residentKey: 'preferred', userVerification: opts.userVerification }`; stores the challenge (`purpose: 'register'`) with the configured TTL. Returns `{ options }`.
- **`verifyRegistration(attestation)`** — extracts the challenge from `attestation.response.clientDataJSON`, consumes it from the challenge store (single-use — `{ error: 'challenge-not-found' }` if missing/already used), then calls `verifyRegistrationResponse` with `expectedOrigin`/`expectedRPID` from `rp` and **`requireUserVerification: opts.userVerification === 'required'`**. With the default `opts.userVerification` (`'preferred'`), user verification is requested during the ceremony above but not enforced here — matching every previously-issued passkey. On success, stores the new credential (id, public key, counter, transports) and returns `{ ok: true, userId, token }` via `createSession`.
- **`getAuthenticationOptions(rawHandle)`** — if a handle is given, validates it and restricts `allowCredentials` to that user's registered credentials (`{ error: 'user-not-found', status: 404 }` if the handle doesn't exist); if omitted, produces a usernameless/discoverable-credential challenge. Calls `generateAuthenticationOptions` with `userVerification: opts.userVerification`. Stores the challenge (`purpose: 'authenticate'`). Returns `{ options }`.
- **`verifyAuthentication(assertion)`** — consumes the matching challenge, looks up the credential by `assertion.id` (`{ error: 'credential-not-found', status: 404 }` if unknown), and calls `verifyAuthenticationResponse` with the stored public key/counter/transports and, again, **`requireUserVerification: opts.userVerification === 'required'`**. On success, persists the authenticator's new signature counter and returns `{ ok: true, userId, user, token }`.

**Security note on `opts.userVerification: 'required'`:** setting this makes `requireUserVerification: true` on both verification calls, so `verifyAuthenticationResponse`/`verifyRegistrationResponse` reject any response whose authenticator didn't perform user verification (PIN, biometric, or device passcode) — not merely user presence (a touch/tap). This is strictly stronger than the `'preferred'` default, but authenticators that never perform UV (common on older security keys) will be rejected outright, including previously-registered passkeys. Only turn it on after confirming your users' authenticators support UV, or alongside a re-registration flow.
